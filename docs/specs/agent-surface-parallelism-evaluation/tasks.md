...
|---|---|---|---|
| Phase 0 — Telemetry + A/B measurement | not started | | Gate: decisions matrix in `decisions.md` is committed BEFORE Phase 0 runs |
...
- [ ] **0.1** Telemetry pull. Query `~/.attune/telemetry/usage.jsonl` for the last 90 days. Output: `phase0-data/multi-workflow-sessions.csv` with one row per session showing workflows invoked + count. Compute `% of sessions with ≥2 analytical workflows`.
