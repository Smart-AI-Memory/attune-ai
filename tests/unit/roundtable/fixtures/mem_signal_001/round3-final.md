...
[tag: 2-1] — Critique A would commit *now* to `wrong/(acted_on+wrong)` as THE single noise ratio; the resolution instead reports it as one reversible field pending chair ratification of whether `ignored` also counts (OQ-1).
...
- Final ratification of whether `ignored` enters the headline health metric — deferred to the chair (OQ-1), not decided here; the reader keeps the choice reversible.
- Mandating a specific Ollama model tag — the requirement is that model+prompt are pinned/versioned/local, not which one (impl choice).
