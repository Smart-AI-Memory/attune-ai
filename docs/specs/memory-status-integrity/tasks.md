...
| 8 | Live receipt with the frequency term active, after an accrual window | receipt | leg 1 done | ACCRUAL STARTED 2026-08-10 01:12Z — live-fire receipt: a real `PersonalMemory.query` appended a real `curated_recall` event (3 stems, surface personal_query) to the live sink; baseline = task 9's age-only run. **Leg 2 OWED ≥2026-08-23 — a CALIBRATION GATE, not a victory lap** (chair-ratified framing, 2026-08-09 close-out): the fold's constants (floor 0.25, log scale) were picked against SYNTHETIC serves with zero real events; recall_digest serving the same top nodes every session may make real counts near-uniform, and a uniform multiplier reorders nothing while looking like it does. Leg 2 must SEGMENT BY `surface` (events carry it), compare queue order vs the task-9 age-only baseline, and if frequency does not genuinely reorder, CUT or reweight the term with task 7's same discipline |
