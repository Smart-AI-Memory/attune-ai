"""Behavioral tests for SecurityAuditAgentSDKWorkflow.

Tests the Agent SDK security audit workflow covering attributes,
...
@pytest.mark.unit
class TestSecurityAuditAgentSDKWorkflowAttributes:
    """Test workflow class attributes."""

    def test_workflow_has_correct_name(self) -> None:
        """Given the workflow class, name is 'security-audit-sdk'."""
        from attune.workflows.security_audit_agent_sdk import SecurityAuditAgentSDKWorkflow

        wf = SecurityAuditAgentSDKWorkflow()
        assert wf.name == "security-audit-sdk"
...
@pytest.mark.unit
class TestSecurityAuditAgentSDKWorkflowSdkUnavailable:
    ...
    async def test_execute_returns_failure_when_sdk_unavailable(self) -> None:
        ...
        with patch("attune.workflows.security_audit_agent_sdk._SDK_AVAILABLE", False):
...
@pytest.mark.unit
class TestSecurityAuditAgentSDKWorkflowExecution:
    ...
    async def test_execute_returns_success_on_valid_sdk_response(self) -> None:
        ...
        sample_text = (
            "## Summary\n"
            "Security score: 72/100. Several medium-severity issues found.\n\n"
            "## Security\n"
            "- CRITICAL: eval() usage in src/utils.py:45\n"
            "- HIGH: Hardcoded API key in config.py:12\n"
            "- MEDIUM: Missing auth check on /admin endpoint\n"
            "- LOW: Debug logging exposes internal paths\n\n"
            "## Suggestions\n"
            "1. Replace eval() with ast.literal_eval() immediately\n"
        ...
        with (
            patch("attune.workflows.security_audit_agent_sdk._SDK_AVAILABLE", True),
            patch(
                "attune.workflows.security_audit_agent_sdk.claude_agent_sdk",
                mock_sdk,
            ),
        ):
            from attune.workflows.security_audit_agent_sdk import (
...
@pytest.mark.unit
class TestSecurityAuditAgentSDKWorkflowDepth:
...
@pytest.mark.unit
class TestSecurityAuditAgentSDKWorkflowSubagents:
    ...
    def test_four_subagents_defined(self) -> None:
        ...
        from attune.workflows.security_audit_agent_sdk import _SUBAGENT_NAMES
