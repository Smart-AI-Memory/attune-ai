---
title: "Build Your First AI Workflow with Attune"
date: "2026-02-23"
author: "Patrick Roebuck"
excerpt: "Go from pip install to running your first cost-optimized Claude workflow in under 5 minutes. Then build your own in four files."
tags: ["tutorial", "getting-started", "workflows", "claude", "anthropic", "python"]
published: true
---

# Build Your First AI Workflow with Attune

Attune AI is an open-source framework that gives you pre-built developer workflows for Claude — security audits, code reviews, test generation, and more — all from your terminal. Each workflow automatically routes tasks to the right Claude model so you're not paying Opus prices for Haiku-level work.

This tutorial gets you from zero to running in under 5 minutes, then shows you how to build your own workflow in four files.
...
```bash
# Security audit — scans for eval, path traversal, injection risks
attune workflow run security-audit --path ./src

# Code review — quality, style, and best practice analysis
attune workflow run code-review --path ./src

# Bug prediction — finds patterns likely to produce bugs
attune workflow run bug-predict --path ./src
```

That's it. Each workflow runs multiple stages, automatically routing cheap tasks to Haiku and quality-critical tasks to Opus. You'll see the cost savings in the output.
...
If you use Claude Code, you don't need to memorize commands. Just type `/attune` and describe what you need:

- "find security vulnerabilities in my auth code"
- "generate tests for src/models/"
- "review my last commit"

Attune asks clarifying questions, then runs the right workflow.

---

## Part 2: Build Your Own Workflow (15 Minutes)

Every Attune workflow follows the same four-file pattern. Let's build a documentation generator as an example.
...
- **`tier_map`** — assigns a Claude model to each stage. `CHEAP` = Haiku ($0.80/M tokens), `CAPABLE` = Sonnet ($3/M), `PREMIUM` = Opus ($15/M).
- **`run_stage()`** — the only method you implement.

This workflow costs roughly **$0.38 per run** vs **$0.90 on Opus alone** — a 58% reduction. With Anthropic's automatic prompt caching (cached tokens cost 10% of standard price), savings compound further.

### File 2: Skill Definition (Natural Language)

Create `plugin/skills/docs/SKILL.md` (`mkdir -p plugin/skills/docs`):
...
Call the workflow with the scoped parameters.
...
The `triggers` array is what connects natural language to your workflow. When someone types "generate docs for src/models", the router matches on "docs" and activates this skill.
...
You've installed Attune, run a built-in workflow, and built your own in four files. From here:
...
- **Read the [full tutorial](https://smartaimemory.com/framework-docs/tutorials/build-a-workflow/)** for deeper patterns

Every built-in workflow follows the same four-file pattern you just learned. Browse the [source](https://github.com/Smart-AI-Memory/attune-ai/tree/main/src/attune/workflows) for real-world examples.
