"""``DependencyCheckSource`` — LLM adapter wrapping ``DependencyCheckWorkflow``.

Mirrors the :class:`BugPredictSource` / :class:`SecurityAuditSource`
pattern: constructs a fresh :class:`DependencyCheckWorkflow` per
...
@dataclass
class DependencyCheckSource(LLMSource):
    """Discovery-sweep adapter for :class:`DependencyCheckWorkflow`.
    ...
    async def discover(self, paths: list[str], budget_usd: float) -> list[Finding]:
        ...
        for path in paths:
            ...
            try:
                ...
                logger.exception("dependency-check execute() failed for %s", path)
