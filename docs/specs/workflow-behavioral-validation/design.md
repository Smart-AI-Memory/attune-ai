...
- The projector selects, per workflow, the record with the **latest
  Git commit order** across the full committed record set, and rebuilds
  the whole table from scratch each run — it does NOT diff against the
  existing table. Commit order (the sequence in which records were
  committed to the tracked history) is the authoritative monotonic key:
  it cannot be skewed by a machine clock, and for an append-only tracked
  set it is total and deterministic. (round-3 finding: `ran_at` is NOT
  the ordering key — a future-skewed wall-clock stamp would otherwise
  win indefinitely; `ran_at` and `git_sha` are informational provenance
  only.) Because the set is append-only and re-projection is idempotent,
  there is no local-overwrite path to guard.
...
- The set of carved driver files is **generated** from the registry each
  ...
  automatically — no human toggles a static list.
- **Write-back mechanism (round-3 finding 2).** Revocation only happens
  in-tree if the new record AND the regenerated carve config are
  actually committed. So the probe job's record-write + config-regen is
  an **always-runs step, independent of the pass/fail verdict**: on
  every run (pass OR fail OR crash) the job commits the new run-record
  and opens/updates an automated PR with the regenerated carve config.
  The verdict blocks the release; the write-back happens regardless — a
  blocking failure must NOT be allowed to leave the last passing record
  and a stale carve checked in. A drift guard fails CI if the committed
  carve set does not match what the current registry would generate, so
  a missed write-back is caught rather than silently leaving a stale
  carve.
- A **positive, per-file** enforcer requires each extracted seam-helper
  module to meet the 85% floor **individually** (findings: round-1 #4,
  round-3 #3). Aggregated run coverage is insufficient — unrelated tests
  or combined helper coverage could satisfy an aggregate target while a
  specific helper is under-covered. So: each seam-helper is its own
  codecov per-path (or per-flag) target with its own 85% threshold, and
  the spec carries an explicit **source→helper ownership map** (which
  helper module each workflow's seams live in) so the gate measures the
  right file. "Seams keep the floor" is a check that FAILS when any one
  seam-helper drops below 85%.
...
- Registry file format (`.md` table vs `.csv`) and the exact run-record
  schema fields.
- Per-workflow: split (carve the driver) vs no-carve (full floor +
  additive probe, when a clean split is impossible) — recorded as each
  is worked. Whole-module advisory-retreat is NOT an option (D6/D8).
