...
- **Shim vs. archive** for the package (T4) — chair decision, now
  with numbers in hand (2026-07-21, pypistats without-mirrors):
  last day 7 / last week 86 / last month 1,540 downloads; latest
  0.25.0, 42 releases. Caveat: attune-ai's own CI (weekly
  freshness `pip install -e '.[author]'`) and `[author]`-extra
  installs contribute, plus PyPI bot noise — not evidence of
  human adopters.
