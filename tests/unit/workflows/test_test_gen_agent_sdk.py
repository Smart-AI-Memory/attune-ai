"""Behavioral tests for TestGenAgentSDKWorkflow.

Tests the Agent SDK test generation workflow covering attributes,
...
@pytest.mark.unit
class TestTestGenAgentSDKWorkflowAttributes:
    """Test workflow class attributes."""

    def test_workflow_has_correct_name(self) -> None:
        """Given the workflow class, name is 'test-gen-sdk'."""
        from attune.workflows.test_gen_agent_sdk import TestGenAgentSDKWorkflow

        wf = TestGenAgentSDKWorkflow()
        assert wf.name == "test-gen-sdk"
...
@pytest.mark.unit
class TestTestGenAgentSDKWorkflowSdkUnavailable:
    ...
    async def test_execute_returns_failure_when_sdk_unavailable(self) -> None:
        ...
        with patch("attune.workflows.test_gen_agent_sdk._SDK_AVAILABLE", False):
...
@pytest.mark.unit
class TestTestGenAgentSDKWorkflowExecution:
    """Test successful execution with mocked SDK."""

    @pytest.mark.asyncio
    async def test_execute_returns_success_on_valid_sdk_response(self) -> None:
        """Given a mocked SDK returning test gen text, execute returns success."""
        sample_text = (
            "## Summary\n"
            "Analyzed 25 functions, designed 40 test cases, wrote 3 test files.\n\n"
            "## Coverage\n"
            "- Current coverage: 72% overall\n"
            "- New tests target uncovered modules\n\n"
            "## Test Gaps\n"
            "- src/attune/orchestration/tools/security.py lacks edge case tests\n"
            "- src/attune/memory/encryption.py missing error path coverage\n\n"
            "## Suggestions\n"
            "- Add parametrized tests for validation functions\n"
            "- Test error handling paths in encryption module\n"
        )
        ...
        with (
            patch("attune.workflows.test_gen_agent_sdk._SDK_AVAILABLE", True),
            patch(
                "attune.workflows.test_gen_agent_sdk.claude_agent_sdk",
                mock_sdk,
            ),
        ):
            from attune.workflows.test_gen_agent_sdk import (
...
@pytest.mark.unit
class TestTestGenAgentSDKWorkflowDepth:
...
@pytest.mark.unit
class TestTestGenAgentSDKWorkflowSubagents:
    ...
    def test_three_subagents_defined(self) -> None:
        ...
        from attune.workflows.test_gen_agent_sdk import _SUBAGENT_NAMES
