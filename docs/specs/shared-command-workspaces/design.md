...
**Status:** authored from approved requirements; awaiting task execution
approval.
...
class CommandWorkspaceAdapter(Protocol):
    ...
    def create(self, intake: Mapping[str, object]) -> CommandState: ...
    def view(self, state: CommandState) -> WorkspaceView: ...
    def legal_actions(self, state: CommandState) -> tuple[str, ...]: ...
    def apply(self, state: CommandState, action: CollectedAction) -> Transition: ...
    def receipt(self, state: CommandState) -> WorkspaceView | None: ...
```

Names are illustrative; Task 1 pins the smallest interface against real
Fix, Roundtable, and Spec call sites. The host owns a versioned envelope:
...
(workspace_id, revision, adapter_id/version, action_id,
 normalized_payload_digest, checkpoint_id, nonce)
```

Any intervening mutation invalidates the binding. Read-only actions can omit
explicit confirmation but still require membership in the server-issued
legal action set.
...
surfaces use bounded batches or pagination; if the host dialog cannot scroll,
the portable fallback must remain operable. The seven-item truncated dialog
observed while approving this spec becomes a regression fixture, not an
anecdote.
