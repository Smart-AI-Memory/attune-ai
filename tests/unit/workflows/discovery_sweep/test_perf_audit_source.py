"""Unit tests for :class:`PerfAuditSource`.

Mocks ``PerformanceAuditWorkflow`` at its source module (per the
existing CLAUDE.md lesson on deferred imports). Integration
coverage gated on ``@pytest.mark.integration`` lives in
``tests/integration/test_discovery_sweep_perf_audit_integration.py``.

Same shape as the P2.1 / P2.2 / P2.3 adapter test files — every
Phase 2B adapter is testable from this pattern.
"""
...
class _FakeWorkflowFactory:
    """Hand-rolled stand-in for ``PerformanceAuditWorkflow``."""
        ...
        "findings": [
            {
                ...
                "description": "Loops issue per-user queries instead of joining.",
                "file": "src/views/users.py",
                "line": 42,
                "evidence": "for u in users: u.role = Role.get(u.role_id)",
                "confidence": 0.85,
...
def test_source_appears_in_default_sources() -> None:
    """Wired into ``default_sources()`` per task P2.4 step 5."""
    sources = default_sources()
    matched = [s for s in sources if s.name == SOURCE]
    assert len(matched) == 1
    assert isinstance(matched[0], PerfAuditSource)
...
async def test_constructor_receives_structured_emit_footer() -> None:
    ...
    with patch(
        ...
        await PerfAuditSource().discover(["src/"], budget_usd=1.0)
...
async def test_multiple_paths_yield_per_path_calls_and_concat_findings() -> None:
    ...
    with patch(
        ...
        findings = await PerfAuditSource().discover(["src/a/", "src/b/"], budget_usd=1.0)
...
async def test_empty_paths_returns_source_failure_finding() -> None:
    ...
    with patch(
        ...
        findings = await PerfAuditSource().discover([], budget_usd=1.0)
...
async def test_wrapped_workflow_raise_does_not_abort_other_paths() -> None:
    ...
    with patch(
        ...
        findings = await PerfAuditSource().discover(["bad/", "good/"], budget_usd=1.0)
