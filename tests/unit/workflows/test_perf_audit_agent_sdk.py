"""Behavioral tests for PerfAuditAgentSDKWorkflow.

Tests the Agent SDK performance audit workflow covering attributes,
...
class TestPerfAuditAgentSDKWorkflowExecution:
    ...
    async def test_execute_returns_success_on_valid_sdk_response(self) -> None:
        ...
        sample_text = (
            "## Summary\n"
            "Performance health score: 78/100. Some optimization opportunities.\n\n"
            "## Performance\n"
            "- Blocking I/O in 3 async functions\n\n"
            "## Complexity\n"
            "- 5 functions exceed cyclomatic complexity threshold\n\n"
            "## Optimization\n"
            "- Replace sorted()[:N] with heapq.nlargest in 2 locations\n\n"
            "## Suggestions\n"
            "- Add caching for repeated file reads\n"
            "- Convert list comprehensions to generators in hot paths\n"
        )
        ...
        with (
            ...
            from attune.workflows.perf_audit_agent_sdk import (
...
@pytest.mark.unit
class TestPerfAuditAgentSDKWorkflowDepth:
...
@pytest.mark.unit
class TestPerfAuditAgentSDKWorkflowSubagents:
    ...
    def test_three_subagents_defined(self) -> None:
        ...
        from attune.workflows.perf_audit_agent_sdk import _SUBAGENT_NAMES
