...
| 2 | Instrument curated serve events: `PersonalMemory.query` hits and `recall_digest` fetches emit a `curated_recall` event naming served stems | attune-ai | pending | via the existing `log_memory_event` sink (schema `v`, `DO_NOT_TRACK`-respecting, best-effort/fail-open); stems only, never content — the event is a counter, not a copy |
...
| 4 | `serve_counts` reader: per-stem serve frequency over a window (default 30d) from `memory_events.jsonl` | attune-ai | pending | pure read-side library; malformed lines skipped; missing file → all-zero (fail-open); window + path injectable for hermetic tests |
