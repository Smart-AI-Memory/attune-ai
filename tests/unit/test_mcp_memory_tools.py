...
class TestToolRegistration:
    ...
    def test_tools_list_returns_at_least_core_count(self, server: AttuneMCPServer):
        """Core tools (55) are always registered; Redis may add its session tools."""
        ...
        assert len(tools) >= 55
        assert {"elicitation_route_form", "prompt_refinement"}.issubset(tool_names)
        ...
        if redis_tools.issubset(tool_names):
            ...
            expected = 66 if "session_memory_status" in tool_names else 61
            assert (
                len(tools) == expected
