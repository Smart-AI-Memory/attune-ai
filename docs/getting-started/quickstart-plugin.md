...
The plugin ships <!-- cap:skill_count -->**28 auto-triggering skills**<!-- /cap -->. You don't memorize commands —
...
| Add the full MCP toolset <!-- cap:mcp_registered_tool_count -->(59 tools)<!-- /cap --> + CLI | [MCP Integration](mcp-integration.md) |
...
!!! note "Plugin vs. package"
    The **plugin** gives you the <!-- cap:skill_count -->28 natural-language skills<!-- /cap --> with zero setup.
    Installing the **Python package** (`pip install attune-ai`) adds the
    `attune` CLI, the MCP server, and <!-- cap:mcp_registered_tool_count -->59 MCP tools<!-- /cap --> on top. You can start with
