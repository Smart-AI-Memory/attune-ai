...
class TestRedisShortTermMemoryProtocol:
    """Verify RedisShortTermMemory satisfies MemoryBackend.

    Uses mock mode so no Redis server is needed.

    NOTE: RedisShortTermMemory does NOT yet conform to the
    MemoryBackend protocol. Its API uses credentials-based
    access (stash/retrieve require AgentCredentials) and
    is missing several protocol methods (delete, keys,
    is_connected, close, supports_*).

    These tests are marked xfail to document the gap.
    Phase 2 of the Redis plugin plan will add an adapter
    ...
    def redis_mem(self):
        ...
        try:
            ...
            pytest.skip("Redis backend not installed")

    @pytest.mark.xfail(
        reason="Phase 2: Redis backend needs protocol adapter",
        strict=True,
    )
    def test_isinstance_check(self, redis_mem):
        """RedisShortTermMemory must pass runtime protocol check."""
        assert isinstance(redis_mem, MemoryBackend)

    def test_get_stats_returns_dict(self, redis_mem):
        """get_stats() already returns a dict."""
        stats = redis_mem.get_stats()
        assert isinstance(stats, dict)

    def test_ping_works_as_health_check(self, redis_mem):
        """ping() exists but is_connected() is needed for protocol."""
        assert redis_mem.ping() is True

    @pytest.mark.xfail(
        reason="Phase 2: stash() currently requires credentials arg",
        strict=True,
    )
    def test_stash_protocol_signature(self, redis_mem):
        """stash() must accept (key, value, ttl, agent_id)."""
        result = redis_mem.stash("k1", {"v": 1})
        assert result is True

    @pytest.mark.xfail(
        reason="Phase 2: missing delete(), is_connected(), close(), supports_*()",
        strict=True,
    )
    def test_missing_protocol_methods(self, redis_mem):
        """Redis backend must expose all protocol methods."""
        assert hasattr(redis_mem, "delete")
        assert hasattr(redis_mem, "is_connected")
        assert hasattr(redis_mem, "close")
        assert hasattr(redis_mem, "supports_realtime")
        assert hasattr(redis_mem, "supports_distributed")


# =========================================================================
# AMSMemoryBackend protocol compliance
# =========================================================================
