...
# FAQ: Why do I get `ImportError` (import X inside a try block + except X.SomeError in the except clause crashes with UnboundLocalError when the import fails)?
...
```
.clusterfuzzlite/fuzz_config_parsing.py
```

## Related Topics
- **Error**: Detailed error: `import X` inside a `try` block + `except
