"""Agent SDK Documentation Generation Workflow.

Delegates documentation generation to the Claude Agent SDK, using three
specialized subagents (outline-planner, content-writer, polish-reviewer)
and synthesizing their output into a unified WorkflowResult.
...
class DocGenAgentSDKWorkflow(BaseWorkflow):
    """Documentation generation workflow powered by the Claude Agent SDK.
    ...
    Usage::
        ...
        workflow = DocGenAgentSDKWorkflow()
        result = await workflow.execute(path="src/", depth="standard")
    """

    name = "doc-gen-sdk"
    description = "Agent SDK-powered documentation generation with 3 specialized subagents"
    stages = ["agent-gen"]
