...
| 4 | Codex live MCP canary | UNPROBED | — |
| 5 | Claude Code hook canary | PASS (live) | 2026-07-22 |
| 6 | Antigravity/Gemini probe | UNPROBED | — |
...
## 4 — Codex live MCP canary — UNPROBED (honest)

Requires a live Codex session; cannot be produced from Claude Code.
Next Codex session: `session_memory_capture` a dated canary →
`session_memory_recall` it → `session_memory_forget` it → append the
transcript here. (The 2026-07-22 diagnosis already verified generic
...
## 6 — Antigravity/Gemini probe — UNPROBED (honest)

Requires a live Antigravity session. Probe: does the client expose
the attune MCP server? If yes, run the receipt-4 canary flow; if no,
record an explicit `unsupported` row here. Automatic lifecycle hooks
are NOT promised on Codex or Antigravity (D2/R5).
