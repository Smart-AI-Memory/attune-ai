"""Agent SDK Test Generation Workflow.

Delegates test generation to the Claude Agent SDK, using three
specialized subagents (function-identifier, test-designer,
test-writer) and synthesizing their output into a unified
WorkflowResult.
...
You are a test generation orchestrator. Analyze the codebase at {path} \
using the three specialized subagents below. Each subagent should focus \
on its domain and produce structured markdown output.
...
class TestGenAgentSDKWorkflow(BaseWorkflow):
    """Test generation workflow powered by the Claude Agent SDK.

    Delegates all analysis to three Agent SDK subagents rather
    than using the mixin stage system. Each subagent focuses on
    a specific phase of test generation (identification, design,
    writing). The orchestrator synthesizes output into a unified
    report.

    Usage::

        workflow = TestGenAgentSDKWorkflow()
    ...
    async def execute(self, **kwargs: Any) -> WorkflowResult:
        ...
        try:
            ...
            logger.exception("Agent SDK test generation failed: %s", type(exc).__name__)
