...
@pytest.mark.unit
class TestResearchSynthesisAgentSDKWorkflowSdkUnavailable:
    ...
    async def test_execute_returns_failure_when_sdk_unavailable(self) -> None:
        ...
        with patch("attune.workflows.research_synthesis_agent_sdk._SDK_AVAILABLE", False):
...
class TestResearchSynthesisAgentSDKWorkflowExecution:
    ...
    async def test_execute_returns_success_on_valid_sdk_response(self) -> None:
        ...
        with (
            patch("attune.workflows.research_synthesis_agent_sdk._SDK_AVAILABLE", True),
            patch(
