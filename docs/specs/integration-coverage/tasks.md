...
**Status:** in progress (Phase 2 added + approved 2026-06-11; task 7 done) — Phase 0 complete (see [phase0-findings.md](phase0-findings.md)): decision **GO, reframed** — revive the existing 351-test suite, don't build new infra. Phase 1 **complete**: rot pruned (#703), green subset wired to CI (#704), 16-failure backlog cleared + job promoted to the full no-auth suite (295 passed / 0 failed — see [phase1-triage.md](phase1-triage.md)). Nightly auth job shipped: `integration-auth.yml` (schedule + workflow_dispatch) runs the auth bucket — env-gated discovery_sweep files + `test_llm_integration.py` (`-k "discovery_sweep or llm_integration"`; the 6 `*_with_auth` files were retired in #727) — with the real `ANTHROPIC_API_KEY` secret and `ATTUNE_MAX_BUDGET_USD=10`. First auth-bucket dispatch triaged in [auth-run-triage.md](auth-run-triage.md). Phase 2 task 7 (2.1 surface inventory) **complete** (2026-06-11): see [surface-inventory.md](surface-inventory.md) — known-worst confirmed: 21 MCP workflow tools have no exercise past validation and no MCP tool has LIVE exercise. Remaining: tasks 8–10; promote the no-auth CI job to a required check after a few weeks green.
---
