"""Agent SDK Dependency Check Workflow.

Delegates a full dependency check to the Claude Agent SDK, using two
specialized subagents (inventory-assessor, update-advisor) and
synthesizing their findings into a unified WorkflowResult.
...
You are a dependency check orchestrator. Analyze the project at {path} \
using the two specialized subagents below. Each subagent should focus on \
...
Be thorough but concise. Cite package names and version numbers.\
"""


class DependencyCheckAgentSDKWorkflow(BaseWorkflow):
    """Dependency check workflow powered by the Claude Agent SDK.

    Delegates all analysis to two Agent SDK subagents rather
    than using the mixin stage system. The inventory-assessor
    inventories all dependencies and checks for outdated or
    vulnerable packages. The update-advisor assesses update
    urgency and creates a prioritized update plan.

    Usage::

        workflow = DependencyCheckAgentSDKWorkflow()
        result = await workflow.execute(path=".", depth="standard")
    """

    name = "dependency-check-sdk"
    description = "Agent SDK-powered dependency check with 2 specialized subagents"
    stages = ["agent-check"]
    ...
    async def execute(self, **kwargs: Any) -> WorkflowResult:
        ...
        try:
            result_text = await self._run_agent_check(resolved_path, max_turns)
    ...
    async def _run_agent_check(self, resolved_path: str, max_turns: int) -> str:
