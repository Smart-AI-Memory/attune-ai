...
class TestRedisStreams:
    ...
    def test_stream_append_adds_agent_id_and_timestamp(self, memory, contributor_creds):
        ...
        memory.stream_append("audit", {"action": "test_action"}, contributor_creds)
    ...
    def test_stream_append_multiple_entries(self, memory, contributor_creds):
        ...
        for i in range(5):
            memory.stream_append("events", {"event_number": i}, contributor_creds)
    ...
    def test_stream_append_max_len_trims_old_entries(self, memory, contributor_creds):
        ...
        for i in range(20):
            memory.stream_append("limited_stream", {"index": i}, contributor_creds, max_len=10)

        # Read all entries
        entries = memory.stream_read("limited_stream", contributor_creds, count=100)
    ...
    def test_stream_read_returns_entries_in_order(self, memory, contributor_creds):
        ...
        for i in range(5):
            memory.stream_append("test_stream", {"index": i}, contributor_creds)
    ...
    def test_stream_read_with_start_id_filters_entries(self, memory, contributor_creds):
        ...
        entry_ids = []
        for i in range(5):
            entry_id = memory.stream_append("filtered_stream", {"index": i}, contributor_creds)
    ...
    def test_stream_read_new_returns_empty_in_mock_mode(self, memory, contributor_creds):
        ...
        entries = memory.stream_read_new("test_stream", contributor_creds, block_ms=0, count=10)
