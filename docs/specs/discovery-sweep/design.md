...
**Resolved 2026-05-13 (Phase 1):** `asyncio.gather(*tasks, return_exceptions=True)` across sources. Each adapter is `async def discover(...)`; a crashed source becomes one questions entry (`SOURCE_CRASHED` reason), not a failed sweep.
...
**Resolved 2026-05-13 (Phase 1):** Severity rank `critical > high > medium > low > info`; queue threshold = `medium`; confidence threshold = `0.5`. Tune after first dogfood run.
...
**Resolved 2026-05-13 (Phase 1):** Yes — add `discovery-sweep` to `PATH_ARG_REGISTRY` in `src/attune/ops/data.py` as Category A (`path` kwarg) under task P1.7. Required for ops-dashboard scope picker.
