...
Usage:
    ...
    python -m attune.patterns.git_extractor

    # Manual extraction for recent commits
    python -m attune.patterns.git_extractor --commits 5
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "log",
                    "--first-parent",
                    # Implied by --first-parent since git 2.29 ("the
                    # --first-parent option has been made to imply -m",
                    # RelNotes/2.29.0); explicit so merge commits keep their
                    # first-parent patch on older gits too (verified live:
...
Examples:
  ...
  python -m attune.patterns.git_extractor --staged

  # Save detected patterns
  python -m attune.patterns.git_extractor --save
        """,
    )
