"""Behavioral tests for BugPredictAgentSDKWorkflow.

Tests the Agent SDK bug prediction workflow covering attributes,
...
class TestBugPredictAgentSDKWorkflowAttributes:
    ...
    def test_workflow_has_stages_list(self) -> None:
        """Given the workflow class, stages list contains 'agent-predict'."""
        from attune.workflows.bug_predict_agent_sdk import BugPredictAgentSDKWorkflow

        wf = BugPredictAgentSDKWorkflow()
        assert isinstance(wf.stages, list)
        assert "agent-predict" in wf.stages
...
class TestBugPredictAgentSDKWorkflowExecution:
    ...
    async def test_execute_returns_success_on_valid_sdk_response(self) -> None:
        """Given a mocked SDK returning prediction text, execute returns success."""
        sample_text = (
            "## Summary\n"
            "Risk score: 72/100. Several high-risk patterns detected.\n\n"
            "## Bugs\n"
            "- HIGH: Potential null reference in parser.py:45\n"
            "- MEDIUM: Broad exception handler in executor.py:120\n"
            "- LOW: Missing type annotation in utils.py:30\n\n"
            "## Suggestions\n"
            "- Add null checks before accessing parsed results\n"
        ...
        with (
            ...
            from attune.workflows.bug_predict_agent_sdk import (
...
@pytest.mark.unit
class TestBugPredictAgentSDKWorkflowDepth:
...
@pytest.mark.unit
class TestBugPredictAgentSDKWorkflowSubagents:
    ...
    def test_three_subagents_defined(self) -> None:
        ...
        from attune.workflows.bug_predict_agent_sdk import _SUBAGENT_NAMES
