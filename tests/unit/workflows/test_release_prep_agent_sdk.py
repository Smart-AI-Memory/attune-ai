...
@pytest.mark.unit
class TestReleasePrepAgentSDKWorkflowSdkUnavailable:
    ...
    async def test_execute_returns_failure_when_sdk_unavailable(self) -> None:
        ...
        with patch("attune.workflows.release_prep_agent_sdk._SDK_AVAILABLE", False):
...
class TestReleasePrepAgentSDKWorkflowExecution:
    ...
    async def test_execute_returns_success_on_valid_sdk_response(self) -> None:
        ...
        with (
            patch("attune.workflows.release_prep_agent_sdk._SDK_AVAILABLE", True),
            patch(
